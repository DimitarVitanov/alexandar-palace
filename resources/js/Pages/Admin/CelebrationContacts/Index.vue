...
const applyFilters = () => {
    router.get('/admin/celebration-contacts', {
...
const deleteContact = (contact) => {
    if (confirm('Delete this contact request?')) {
        router.delete(`/admin/celebration-contacts/${contact.id}`);
    }
};

const saveRecipients = () => {
    recipientsForm.post('/admin/celebration-contacts/recipients', {
        onSuccess: () => {
...
<template>
    <AdminLayout>
        <template #title>Celebration Inquiries</template>
        <template #actions>
            <button 
        ...
        <div v-if="showSettings" class="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-6">
            ...
            <p class="text-sm text-slate-500 mb-4">
                Enter email addresses (comma-separated) that should receive celebration contact requests.
            </p>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Event Type</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Event Date</th>
                            <th class="px-6 py-4 text-left text-xs font-semibold text-slate-500 uppercase tracking-wider">Guests</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        ...
                        >
                            ...
                            <td class="px-6 py-4">
                                <span v-if="contact.event_type" class="px-2 py-1 text-xs font-medium rounded-full bg-purple-100 text-purple-700">
                            ...
                            <td class="px-6 py-4 text-right">
                                <div class="flex items-center justify-end gap-2">
                                    <Link 
                                        :href="`/admin/celebration-contacts/${contact.id}`" 
