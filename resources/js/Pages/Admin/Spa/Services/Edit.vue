...
const form = useForm({
    name: props.service?.name || { en: '', mk: '' },
    description: props.service?.description || { en: '', mk: '' },
    duration_minutes: props.service?.duration_minutes || 60,
    price: props.service?.price || '',
    discounted_price: props.service?.discounted_price || '',
    category: props.service?.category || 'massage',
    featured_image: props.service?.featured_image || '',
    max_guests: props.service?.max_guests || 1,
...
const submit = () => {
    form.put(`/admin/spa/services/${props.service.id}`);
};
</script>

<template>
    <AdminLayout>
        <template #title>Edit Spa Service</template>
        <template #actions>
            <Link href="/admin/spa/services" class="inline-flex items-center px-4 py-2 bg-white text-slate-700 text-sm font-medium rounded-lg border border-slate-300 hover:bg-slate-50 transition-colors mr-3">
