...
const deleteContact = () => {
    if (confirm('Delete this contact request?')) {
        router.delete(`/admin/congress-contacts/${props.contact.id}`);
    }
};
...
<template>
    <AdminLayout>
        <template #title>Contact Request</template>
        <template #actions>
            <Link 
                href="/admin/congress-contacts" 
            ...
            <div class="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden">
                ...
                <div class="px-6 py-4 bg-slate-50 border-b border-slate-200">
                    <div class="flex items-center justify-between">
                        <div>
                            <h2 class="text-xl font-semibold text-slate-800">{{ contact.subject }}</h2>
