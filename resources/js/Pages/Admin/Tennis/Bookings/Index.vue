...
const applyFilters = () => {
    router.get('/admin/tennis/bookings', {
        search: search.value || undefined,
        status: status.value || undefined,
        court_id: courtId.value || undefined,
...
const destroy = (item) => {
    if (confirm('Are you sure you want to delete this booking?')) {
        deleteForm.delete(`/admin/tennis/bookings/${item.id}`);
    }
};

const confirmBooking = (item) => {
    if (confirm('Confirm this booking?')) {
        router.post(`/admin/tennis/bookings/${item.id}/confirm`);
    }
};
...
const cancelBooking = (item) => {
    ...
    if (reason !== null) {
        router.post(`/admin/tennis/bookings/${item.id}/cancel`, { cancellation_reason: reason });
    }
};
...
const getCourtName = (court) => {
    if (!court) return 'N/A';
    return typeof court.name === 'object' ? court.name.en : court.name;
};
</script>

<template>
    <AdminLayout>
        <template #title>Tennis Bookings</template>
        <template #actions>
            <Link href="/admin/tennis/bookings/calendar" class="btn-secondary mr-2">
                <i class="bi bi-calendar3 mr-2"></i> Calendar
            </Link>
            <Link href="/admin/tennis/courts" class="btn-secondary mr-2">
                <i class="bi bi-dribbble mr-2"></i> Courts
            </Link>
            <Link href="/admin/tennis/bookings/create" class="btn-primary">
            ...
            <div class="bg-white rounded-xl p-4 border border-slate-200 shadow-sm">
                <div class="flex items-center gap-3">
                    <div class="w-10 h-10 bg-green-100 rounded-lg flex items-center justify-center">
                        <i class="bi bi-dribbble text-green-600"></i>
                    </div>
                    <div>
        ...
        <div class="bg-white rounded-xl shadow-sm border border-slate-200 p-4 mb-6">
            <div class="grid grid-cols-1 md:grid-cols-5 gap-4">
                <div>
                    <input v-model="search" type="text" placeholder="Search..." class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm focus:ring-2 focus:ring-green-500 focus:border-green-500">
                </div>
                <div>
                ...
                <div>
                    <select v-model="courtId" @change="applyFilters" class="w-full px-3 py-2 border border-slate-300 rounded-lg text-sm">
                        <option value="">All Courts</option>
                        <option v-for="court in courts" :key="court.id" :value="court.id">{{ getCourtName(court) }}</option>
                    </select>
                </div>
                <div>
                ...
                <table class="w-full">
                    <thead>
                        <tr class="bg-slate-50 border-b border-slate-200">
                            ...
                            <th class="px-4 py-3 text-center text-xs font-semibold text-slate-500 uppercase">Players</th>
                    ...
                    <tbody class="divide-y divide-slate-100">
                        <tr v-if="!bookings?.data?.length">
                            <td colspan="8" class="px-6 py-12 text-center text-slate-500">
                                ...
                                No tennis bookings found.
                            </td>
                        </tr>
                        <tr v-for="item in bookings.data" :key="item.id" class="hover:bg-slate-50 transition-colors">
                            <td class="px-4 py-3">
                                <span class="font-mono text-sm text-green-600 font-medium">{{ item.booking_reference }}</span>
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-slate-600 text-sm">
                                {{ getCourtName(item.tennis_court) }}
                            </td>
                            <td class="px-4 py-3">
                            ...
                            <td class="px-4 py-3 text-center text-slate-600 text-sm">
                                <i class="bi bi-people mr-1"></i> {{ item.players }}
                            </td>
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex items-center justify-end gap-1">
                                    ...
                                    <Link :href="`/admin/tennis/bookings/${item.id}`" class="p-1.5 text-slate-500 hover:text-green-600 hover:bg-green-50 rounded-lg" title="View">
                                        <i class="bi bi-eye"></i>
                                    </Link>
